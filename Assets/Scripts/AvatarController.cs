...
public class AvatarController : MonoBehaviour
{
    ...
    {
        // Wait until poseDetection.keypoints is populated and has at least 33 keypoints
    ...
    private void FindAndCacheBones()
    {
        ...
        {
            Bone headBone = bones[NECK];
            headBone.InitRotation = headBone.Transform.rotation;
            var lookDirection = bones[NOSE].Transform.position - headBone.Transform.position;
            headBone.InverseRotation = Quaternion.Inverse(Quaternion.LookRotation(lookDirection)) * headBone.InitRotation;
        }

        // Hand Specific Inverse Setup 
        if (bones.ContainsKey(LEFT_HAND))
        {
            ...
            Vector3 palmNormal = TriangleNormal(lHandBone.Transform.position, lPinky.Transform.position, lIindex.Transform.position); //up direction from palm
            Vector3 midHand = (lIindex.Transform.position + lPinky.Transform.position) / 2.0f;
            Vector3 lForwardDir = midHand - lHandBone.Transform.position; // forward direction

            // Store initial rotation
            lHandBone.InitRotation = lHandBone.Transform.rotation;

            // Calculate inverse rotation
            lHandBone.InverseRotation = Quaternion.Inverse(Quaternion.LookRotation(lForwardDir,palmNormal)) * lHandBone.InitRotation;
        }
        ...
        if (bones.ContainsKey(RIGHT_HAND))
        {
            ...
            // Calculate palm normal and midpoint between index and pinky
            Vector3 palmNormal = TriangleNormal(rHandBone.Transform.position, rPinky.Transform.position, rIndex.Transform.position); //up direction from palm
            Vector3 midHand = (rIndex.Transform.position + rPinky.Transform.position) / 2.0f;
            Vector3 rForwardDir = midHand - rHandBone.Transform.position; // forward direction

            // Store initial rotation
            rHandBone.InitRotation = rHandBone.Transform.rotation;

            // Calculate inverse rotation
            rHandBone.InverseRotation = Quaternion.Inverse(Quaternion.LookRotation(rForwardDir,palmNormal)) * rHandBone.InitRotation;
        }
    ...
    void Update()
    {
        ...
        keypoints3D = poseDetection.keypoints; 
        // --- 1. Update Bone Keypoint Positions ---
        UpdateBoneKeypointPositions(keypoints3D);

        // Upper body segments: head->hips.
        var dist1 = Vector3.Distance(bones[HEAD].KeypointPosition, bones[HIPS].KeypointPosition);
        ...
        var height = dist1 + legHeight;


        // Low pass height filter
        tall = height * 0.7f + prevTall * 0.3f;
        prevTall = tall;

        this.gameObject.transform.localScale = new Vector3(tall, tall, tall); //scale according to human height
        ...
        {
            bones[HIPS].Transform.position = bones[HIPS].KeypointPosition * 1.5F + new Vector3(initPosition.x, initPosition.y, initPosition.z); // 1.5f movement scaling for now
        ...
        var lookDirection = bones[NOSE].KeypointPosition - bones[NECK].KeypointPosition;
        var f = TriangleNormal(bones[NECK].KeypointPosition, GetKeypointOrZero(keypoints3D, LEFT_EAR_INDEX), GetKeypointOrZero(keypoints3D, RIGHT_EAR_INDEX));
        bones[NECK].Transform.rotation = Quaternion.LookRotation(lookDirection, f) * bones[NECK].InverseRotation;
        ...
        Vector3 lPalmNormal = TriangleNormal(lHand.KeypointPosition, lPinky.KeypointPosition, lIndex.KeypointPosition);
        Vector3 lMidHand = (lIndex.KeypointPosition + lPinky.KeypointPosition) / 2.0f;

        // Apply rotation with inverse
        Quaternion leftHandTargetRotation = Quaternion.LookRotation(lMidHand - lHand.KeypointPosition, lPalmNormal) * lHand.InverseRotation;
        lHand.Transform.rotation = Quaternion.Slerp(lHand.Transform.rotation, leftHandTargetRotation, smoothing);


        Bone rHand = bones[RIGHT_HAND];
        Bone rIndex = bones[RIGHT_INDEX_BASE];
        Bone rPinky = bones[RIGHT_PINKY_BASE];

        // Calculate current palm normal from keypoints
        Vector3 rPalmNormal = TriangleNormal(rHand.KeypointPosition, rPinky.KeypointPosition, rIndex.KeypointPosition);
        Vector3 rMidHand = (rIndex.KeypointPosition + rPinky.KeypointPosition) / 2.0f;

        // Apply rotation with inverse
        Quaternion RightHandTargetRotation = Quaternion.LookRotation(rMidHand - rHand.KeypointPosition, rPalmNormal) * rHand.InverseRotation;

        rHand.Transform.rotation = Quaternion.Slerp(rHand.Transform.rotation, RightHandTargetRotation, smoothing);
    ...
    void UpdateBoneKeypointPositions(Vector3[] keypoints3D)
    {
        ...
        if (bones.ContainsKey(NECK)) bones[NECK].KeypointPosition = HeadKeypoint; //nechbone used for moving head, the rigging is wierd
